...
int izx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  ...
  //  adr = adr << 8;
  //  adr = adr & conv_bitstr2int(memory[zpl+1],0,7);
...
int zp_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  ...
  //int zp = conv_bitstr2int(memory[t],0,7);
  inc_pc(cpu);
  return adr ;
...
void do_BIT(struct _6510_cpu* cpu, char *mem){
  char regasave[] = "00000000";
  char flags[] = "00000000";

  cp_register(cpu->rega, regasave);

  alu(ALU_OP_AND, cpu->rega, mem, cpu->rega, flags);

  cpu->flags[NFLAG] = flags[NFLAG];
  cpu->flags[VFLAG] = flags[VFLAG];
  cpu->flags[ZFLAG] = flags[ZFLAG];
  
  cp_register(regasave, cpu->rega);
}
